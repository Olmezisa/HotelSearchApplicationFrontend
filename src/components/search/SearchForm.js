...
const SearchForm = ({ onSearch, currency, nationality }) => {
  ...
  const validateForm = () => {
    ...
    if (!selectedLocation) return setError('Lütfen bir konum seçin!') || false;
    if (selectionRange.startDate < today) return setError('Giriş tarihi geçmiş olamaz!') || false;
    if (selectionRange.endDate <= selectionRange.startDate) return setError('Çıkış tarihi, giriş tarihinden sonra olmalı!') || false;

    const diff = Math.ceil((selectionRange.endDate - selectionRange.startDate) / (1000 * 60 * 60 * 24));
    if (diff > 30) return setError('Maksimum kalış süresi 30 gündür!') || false;
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (isSearching || !validateForm()) return;
    setIsSearching(true);
    setError(null);
    ...
    try {
      ...
      const searchParams = {
        ...
        checkIn,
        checkOut,
        roomCriteria: rooms.map(r => ({ adult: r.adults, child: r.children, childAges: Array(r.children).fill(5) })),
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-[#1f1f1fcc] backdrop-blur-md rounded-xl shadow-lg p-4 w-full max-w-5xl mx-auto flex flex-col md:flex-row gap-2 items-center">
      {/* Konum */}
      <div className="relative w-full md:flex-1">
        <div className="flex items-center bg-[#1f1f1fcc] text-white rounded-lg px-3 py-2">
          ...
          <input
            ...
            value={query}
            onChange={(e) => { setQuery(e.target.value); setSelectedLocation(null); setError(null); }}
          />
        ...
        {showSuggestions && (
          <div ref={suggestionsRef} className="absolute z-10 mt-1 w-full bg-white rounded-lg shadow border max-h-60 overflow-y-auto">
            {suggestions.map((sug, idx) => (
              <button key={idx} type="button" onClick={() => handleLocationSelect(sug)} className="w-full text-left px-4 py-2 hover:bg-gray-100">
      ...
      {/* Tarih */}
      <div className="relative w-full md:flex-1">
        <button type="button" onClick={() => setShowCalendar(!showCalendar)} className="flex items-center w-full bg-[#1f1f1fcc] text-white rounded-lg px-3 py-2">
      ...
      {/* Oda */}
      <div className="relative w-full md:flex-1">
        <button ref={roomBtnRef} type="button" onClick={() => setShowRoomBox(!showRoomBox)} className="flex items-center w-full bg-[#1f1f1fcc] text-white rounded-lg px-3 py-2">
              ...
              <div key={idx} className="mb-4">
                <div className="flex justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    <button type="button" onClick={() => handleRoomChange(idx, 'adults', 'decrement')} className="text-white border border-white rounded-full px-2">-</button>
                    <span>{room.adults}</span>
                    <button type="button" onClick={() => handleRoomChange(idx, 'adults', 'increment')} className="text-white border border-white rounded-full px-2">+</button>
                  </div>
                </div>
                <div className="flex justify-between">
                  <span>Çocuk</span>
                  <div className="flex items-center gap-2">
                    <button type="button" onClick={() => handleRoomChange(idx, 'children', 'decrement')} className="text-white border border-white rounded-full px-2">-</button>
                    <span>{room.children}</span>
                    <button type="button" onClick={() => handleRoomChange(idx, 'children', 'increment')} className="text-white border border-white rounded-full px-2">+</button>
                  </div>
                </div>
                {rooms.length > 1 && (
                  <div className="text-right mt-2">
                    <button onClick={() => handleRemoveRoom(idx)} className="text-xs text-red-300 hover:underline">Odayı Kaldır</button>
                  </div>
                )}
              </div>
            ))}
            {rooms.length < 5 && (
              <button type="button" onClick={handleAddRoom} className="text-sm text-blue-300 hover:underline w-full text-center mt-2">+ Oda Ekle</button>
            )}
          </div>
        )}
      </div>
  ...
  <button
    type="submit"
    className="bg-[#1f2937] hover:bg-[#172B66] text-white font-semibold px-6 py-2 rounded-lg w-full"
  >
    {isSearching ? (
      <>
